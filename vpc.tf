...
resource "aws_security_group" "keycloak-access" {
  name = "external-keycloak-access-sg"
  description = "Allows access to keycloak port 8443 from everywhere"
  vpc_id = aws_vpc.vpc.id

  ingress {
    description = "Allow all access to port 8443"
    from_port = 8443
    to_port = 8443
    protocol = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
